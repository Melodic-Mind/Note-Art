...
export class NotesHash {
     ...
     * @param {Note} note 
     */
    getKey({ note, octave, instrument, family }) { return `${instrument}${this.fileNoteLetter({note, family})}${octave}` }

    /**
     * trnasforms notes of type '#' to 'b' in order to access the mp3 file's name
     * @param {Note} note 
     */
    fileNoteLetter({ note, family }) { return notes["b"][notes[family].indexOf(note)] }
