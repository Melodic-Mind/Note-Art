...
describe('Guitar', () => {
  ...
  describe('#play', () => {
    it('should play a note when it is valid', () => {
      const stub = sinon.stub(guitar.cords[0], 'play')
      guitar.play('e4', '4n')
  ...
  describe('#playCord', () => {
    it('should play a note on a string', () => {
      const stub = sinon.stub(guitar.cords[0], 'play')
      guitar.playCord(0, 'E4', '4N')
      expect(stub).to.have.been.calledOnce
      stub.restore()
    })

    it('should do nothing when the note doesn\'t exist in the string', () => {
      expect(() => guitar.playString(0, 'C2', '4N')).to.throw(Error)
    })
  })

  describe('#playMultiple', () => {
    it('should play all notes the fitting cords', () => {
      const s1 = sinon.stub(guitar.cords[0], 'play')
      const s2 = sinon.stub(guitar.cords[1], 'play')
