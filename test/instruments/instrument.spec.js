...
describe('Instrument', () => {
    ...
    describe('#setPath', () => {
        ...
        it('should do nothing when paths already has a key', () => {
            const stub = sinon.stub(ins, 'generatePath').returns(Math.random())
            ins.init('e3', 2)
            const spy = sinon.spy(ins.paths, 'set')
            ins.setPath(new Note('E', 3))
    ...
    describe('#play', () => {
        ...
        it('plays a note when it exists', () => {
            ...
            expect(ins.play('E3')).to.be.undefined
            hasNote.restore()
            getPlayer.restore()
        })
    ...
    describe('#syncAndPlay', () => {
        it('should get the player', () => {
            ...
            ins.syncAndPlay('f5')
            expect(stub).to.have.been.calledWithExactly('F5')
        })
    })
})
